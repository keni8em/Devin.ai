...
4. **Select Jira Epic context**:
   ...
   - If project context was selected: parse the stored context output from step 2 to extract epics for the selected project
...
5. **Get entry content**:
   ...
   - For Activity entries: use the time returned by validate_journal.sh and entry description (project and Jira Epic already selected)
   - For Meeting entries: ask for meeting time (or use time returned by validate_journal.sh if not specified), title, type, duration, and project context
   ...
   - For Daily Notes entries: ask for the time (or use time returned by validate_journal.sh if not specified), project context, Jira Epic context
...
6. **Create or update the journal entry**:
   - Use the date and time returned by validate_journal.sh for all log operations
   ...
   - The scripts handle proper formatting and file placement
